...
The TOV equations are solved using the `solve_ivp` function from the *scipy.integrate module*. The right-hand side of the TOV equations is computed using the function called `tov`. The solution of the TOV equations is then re-scaled to obtain the physical quantities, including the radius, enclosed mass, metric potential, pressure particle number density, and enclosed particle number of the dark matter halo. The optional attributes of the `Rar` class then enable the computation of the other physical variables using the outputs of the `model` function.
