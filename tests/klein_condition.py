...
path_rar = os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'rarmodel'))
...
ax.plot(r, np.exp((halo.metric_potential(r) - nu_0)/2.0)*(halo.mu(r) + m_DM)/(m_DM*(1.0 + beta_0*theta_0)) - 1.0, 
        lw = 0.5, ls = ':', color = '#91430e') 
        #label = r'$\frac{\mathrm{e}^{(\nu(r) - \nu_{0})/2}(\mu(r) + mc^{2})}{mc^{2}(1 + \beta_{0}\theta_{0})} - 1$')
...
fig.savefig('../figures/klein_condition.png', bbox_inches = 'tight')
# =============================================================================================== #