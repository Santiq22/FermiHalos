...
halo = Rar(np.array([m_DM, theta_0, W_0, beta_0]), nu_func=True, cutoff_func=True)
...
ax.plot(r, np.exp((halo.metric_potential(r) - nu_0)/2.0)*(halo.e_c(r) + m_DM)/(m_DM*(1.0 + beta_0*W_0)), 
        lw=3, ls=':', color='#91430e', 
        label=r'$\frac{\mathrm{e}^{(\nu(r) - \nu_{0})/2}(\epsilon_{c}(r) + mc^{2})}{mc^{2}(1 + \beta_{0}W_{0})}$')
